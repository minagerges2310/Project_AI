...
FaceNet for face embedding generation, and MongoDB for storing and retrieving known face embeddings.
The system captures video from an RTSP stream or a webcam, detects faces, recognizes them, and optionally
zooms in on a target face specified by the user.
Classes:
    FaceData: A data structure to store face embeddings and associated names.
    FaceRecognitionSystem: The main class that handles face detection, recognition, and video processing.
Functions:
    main(): Entry point of the application. Prompts the user for a target name and starts the system.
Key Features:
    - Face detection using MediaPipe.
    - Face recognition using FaceNet with pre-trained VGGFace2 weights.
    - MongoDB integration for storing and retrieving known face embeddings.
    - RTSP stream and webcam support for video input.
    - Real-time zoom-in functionality for a specified target face.
    - Logging for debugging and monitoring system behavior.
Usage:
    Run the script and provide the name of the target person when prompted. The system will attempt to
    recognize faces in the video stream and zoom in on the specified target face if detected.
Dependencies:
    - OpenCV
    - MediaPipe
    - NumPy
