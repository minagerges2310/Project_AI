...
MONGO_URI = "mongodb+srv://ah01211293047:1Sc5YkBzDBYORnbA@cluster0.5h4nt.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
FRAME_WIDTH = 640
FRAME_HEIGHT = 480
...
@dataclass
class FaceData:
    encoding: np.ndarray  # Changed from 'embedding' to 'encoding' #here
    name: str
...
class FaceRecognitionSystem:
    ...
    def connect_to_mongodb(self) -> bool:
        ...
        for attempt in range(retries):
            try:
                ...
                self.collection = client["test"]["childrens"]
    ...
    def load_face_data(self) -> bool:
        ...
        if self.collection is None:
            ...
            return False
        try:
            data = list(self.collection.find({}, {"encoding": 1, "name": 1, "_id": 0}))  # Changed from 'embedding' to 'encoding' #here
            self.known_faces = [FaceData(np.array(d["encoding"]), d["name"]) for d in data]  # Changed from 'embedding' to 'encoding' #here
            logging.info(f"Loaded {len(self.known_faces)} face encodings")  # Changed from 'embeddings' to 'encodings' #here
        ...
        try:
            ...
            for detection in results.detections:
                ...
                if face_crop.size > 0:
                    ...
                    with torch.no_grad():
                        encoding = self.facenet(face_tensor).cpu().numpy()[0]  # Changed from 'embedding' to 'encoding' #here
                    ...
                    if self.known_faces:
                        for known_face in self.known_faces:
                            distance = np.linalg.norm(encoding - known_face.encoding)  # Changed from 'embedding' to 'encoding' #here
